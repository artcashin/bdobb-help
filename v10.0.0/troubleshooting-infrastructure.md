...
## Live Quotes / EODHD Backend

**Intraday history requests come back short.**
The EODHD provider caps fine-grained history per request (roughly 120 days at one-minute bars). Narrow your requested window instead of expecting a full year of minute bars in one call. See [[eodhd-data-provider|EODHD Data Provider]].

**A symbol looks frozen and you've confirmed it's not a typo.**
Check the live-quotes service's health endpoint to confirm the underlying upstream feed itself is alive, rather than just the service process. See [[live-quotes|Live Quotes (Live Grid)]] for the app-side symptom and [[troubleshooting-using-bdobb|Using BDOBB]] for the typo check to rule out first.

See [[eodhd-data-provider|EODHD Data Provider]], [[live-quotes|Live Quotes (Live Grid)]], and [[kdb-cache|kdb+ Cache (Optional)]] for full context.
