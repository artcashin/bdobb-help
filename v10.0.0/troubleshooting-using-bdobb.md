...
**A symbol never updates and never errors.**
An unrecognized or mistyped symbol doesn't produce an error — it simply never receives ticks, and sits showing only its initial snapshot forever, indistinguishable at a glance from a very quiet stock. Double-check the symbol spelling first. If the spelling is right and it's still frozen, the underlying feed itself may be down — see [[troubleshooting-infrastructure|Configuring the Infrastructure]] for the service-side health check.

**Forex rows look half-empty or show no trade price.**
Expected. Currency feeds carry only bid/ask, not trades — the displayed price is a computed midpoint, and bid/ask columns are only meaningfully populated for forex symbols.

See [[live-quotes|Live Quotes (Live Grid)]] for full context.

## News Ticker

**The widget shows "live" but nothing is actually arriving.**
Check the [[news-ticker|status dot]] — if it reads "dead," reload the widget. If reloading doesn't fix it, the cause is server-side — see [[troubleshooting-infrastructure|Configuring the Infrastructure]].

See [[news-ticker|News Ticker]] for full context.
