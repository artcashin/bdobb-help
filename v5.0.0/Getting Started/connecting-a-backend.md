...
3. If the backend requires an API key, enter it here. Different backends use this key differently — see [[backends-and-connections|Backends and Connections]] for what the field means per backend type.
...
BDOBB is designed around multiple simultaneous backends — connect several at once, each contributing its own widgets and dashboards to the same app.
