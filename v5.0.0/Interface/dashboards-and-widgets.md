...
A single dashboard can mix widgets from different backends — each contributing its own widgets to the same grid. Each widget only depends on its own backend being reachable; see [[layout-and-navigation|Layout and Navigation]] for how BDOBB isolates failures per widget.
