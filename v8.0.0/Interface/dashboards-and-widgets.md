...
A single dashboard can mix widgets from different backends — a general data widget next to a [[news-ticker|news ticker]] card. Each widget only depends on its own backend being reachable; see [[layout-and-navigation|Layout and Navigation]] for how BDOBB isolates failures per widget.
