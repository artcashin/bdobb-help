...
> **Status: optional, already shipped in openbb-docker v10.0.0.** Unlike [[real-time-chart|Real-Time Chart]] and [[arcticdb-explorer|ArcticDB Explorer]], this isn't unreleased — the backend infrastructure exists today. What's missing is the detailed operator write-up (the "for the tinkerers" recipe): the episode covering it hasn't been drafted from the finished build yet, so exact deployment steps aren't documented here.
...
Without it, [[live-quotes|live quotes]] and the [[eodhd-data-provider|EODHD provider]] work exactly as described on their own pages — nothing about day-to-day usage requires this layer. Deploying it buys two things: repeat queries get answered from memory instead of round-tripping to the vendor, and it becomes the history source the [[real-time-chart|planned real-time chart]] widget will read from when a symbol's chart first opens.
...
---
*Source: "Plan: Episodes 10–12 — the chart, the vault, the wire to the Street" (design doc, approved 2026-08-06) — openbb-docker v10.0.0 is described there as already tagged and unchanged, shipping this layer as the foundation the planned chart reads.*
*See also: [[live-quotes|Live Quotes (Live Grid)]] · [[eodhd-data-provider|EODHD Data Provider]] · [[real-time-chart|Real-Time Chart (Planned)]]*
