...
**Serve vs. Funnel**: Serve keeps a service tailnet-only. Funnel exposes it to the public internet. Only put a service behind Funnel if it authenticates on its own — the [[live-quotes|live quotes]] service and the MCP tool server, for example, are deliberately never funneled, because they carry no auth of their own.
