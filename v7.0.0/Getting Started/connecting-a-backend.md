...
3. If the backend requires an API key, enter it here. Different backends use this key differently — see [[backends-and-connections|Backends and Connections]] for how specific backend types (like the AI agent) use this field.
...
BDOBB is designed around multiple simultaneous backends — a general OpenBB Platform backend and an [[ai-chat|AI agent]] can all be connected at once, each contributing its own widgets and dashboards to the same app.
