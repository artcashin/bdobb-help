...
Symptom-first index of issues in the backend services BDOBB connects to — openbb-docker, Rita/MCP, and Tailscale networking. These are all fixed server-side, not inside the BDOBB app. For app-side issues (settings, install), see [[troubleshooting-using-bdobb|Using BDOBB]].
